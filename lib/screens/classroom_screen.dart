...
class _ClassroomScreenState extends State<ClassroomScreen> {
  ...
  bool get _canViewMyClassroom => _permissions['canViewMyClassroom'] == true;
  ...
  Future<void> _loadFirstPage() async {
    ...
    try {
      final result = await _client.getClassrooms(
        scope: _scope,
        childId: _scope == 'child' ? _selectedChildId : null,
  ...
  Future<void> _loadMore() async {
    if (_loadingMore || !_hasMore || _scope != 'all') return;
  ...
  void _toggleScope(String scope, {int? childId}) {
    final samScope = _scope == scope && _selectedChildId == childId;
    setState(() {
      _scope = samScope ? 'all' : scope;
      _selectedChildId = samScope ? null : childId;
    });
    _loadFirstPage();
  }
  ...
  Future<void> _pickChild() async {
    ...
    if (children.length <= 1) {
      _toggleScope('child', childId: children.isEmpty ? null : children.first.userId);
      return;
    }
    ...
    _toggleScope('child', childId: childId == -1 ? null : childId);
  }
        ...
        actions: [
          if (_canViewMyClassroom)
            IconButton(
              icon: Icon(
                Icons.school,
                color: _scope == 'mine' ? AppColors.primary : null,
              ),
              tooltip: 'My Classroom',
              onPressed: () => _toggleScope('mine'),
            ),
          if (_canViewMyChildClassroom)
            IconButton(
              icon: Icon(
                Icons.family_restroom,
                color: _scope == 'child' ? AppColors.primary : null,
              ),
              tooltip: 'My Child Classroom',
              onPressed: _pickChild,
            ),
          if (_canAdd)
            IconButton(
